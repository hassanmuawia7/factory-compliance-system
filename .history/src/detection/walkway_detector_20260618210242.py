...
def main():
    ...
    # --- THE COOLDOWN LOGIC (The Fix) ---
    last_violation_time = 0
    COOLDOWN_SECONDS = 5  # Wait 5 seconds before logging another violation
    ...
    while cap.isOpened():
        ...
        # --- THE EVENT LOGGING WITH COOLDOWN ---
        current_time = time.time()
        
        if current_frame_violation:
            # Only trigger if the cooldown has expired
            if (current_time - last_violation_time) > COOLDOWN_SECONDS:
                last_violation_time = current_time
                print("\n🚨 NEW VIOLATION DETECTED 🚨")
                print(json.dumps(generate_event(walkway_rule), indent=4))
            else:
                # Optional: Show a subtle on-screen warning that a violation is active but on cooldown
