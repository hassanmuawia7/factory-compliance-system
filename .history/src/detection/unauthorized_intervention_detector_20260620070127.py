...
def detect_green_vest(frame, bbox):
    ...
    # Strict 25% threshold to prevent false positives
    ...
    while cap.isOpened():
        ...
        for box in results[0].boxes:
            ...
            if person_in_any_machine:
                # STRATEGIC FIX: Vest detection temporarily disabled to force violation events
                # has_vest = detect_green_vest(frame, (x1, y1, x2, y2))
                has_vest = False 
                
                print(f"{current_machine_name} | Vest check forced to: {has_vest}")
