...
class ReportGenerator:
    ...
    def generate_csv_report(output_path: str = None) -> str:
        ...
        if output_path is None:
            output_path = f"outputs/compliance_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
    ...
    def generate_json_report(output_path: str = None) -> str:
        ...
        if output_path is None:
            output_path = f"outputs/compliance_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
"""
    ...
    def export_filtered_data(
        ...
        if format == 'csv':
            ...
            output_path = f"outputs/rports{filename}.json"
            os.makedirs(os.path.dirname(output_path), exist_ok=True)
            records = df.to_dict(orient='records')
            with open(output_path, 'w', encoding="utf-8") as f:
                json.dump(records, f, indent=2)
