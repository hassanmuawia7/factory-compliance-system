    ...
    with col3:
        ...
        with col_status1:
            ...
            <div style="text-align: center;">
                ...
                <div style="color: {COLORS['text_muted']}; font-size: 0.85rem;">System Status</div>
            </div>
            """, unsafe_allow_html=True)
            ...
            <div style="display: flex; justify-content: space-between; align-items: start;">
                <div style="flex: 1;">
                    ...
                    <div style="color: {COLORS['text_muted']}; font-size: 0.85rem;">
                        📍 {ts} | Policy: {row['policy_reference']}
                    </div>
                </div>
            </div>
        </div>
...
def render_analytics_section(data: pd.DataFrame):
    ...
    # Behavior Type Distribution (Bar Chart)
    with col_bar:
...
def render_filter_panel() -> Tuple[list, list, tuple, str]:
    ...
    with col2:
        behavior_filter = st.multiselect(
            "📋 Behavior Types",
            sorted(df['behavior_class'].unique().tolist()) if not df.empty else [],
            default=sorted(df['behavior_class'].unique().tolist()) if not df.empty else [],
...
def render_violation_table(data: pd.DataFrame):
    """Render enhanced violation table with styled severity badges."""
    ...
    display_df['policy_reference'] = display_df['policy_reference'].fillna('N/A')
    
    # Select columns to display
    display_columns = [
        'timestamp', 'event_id', 'severity', 'behavior_class',
        'description', 'policy_reference', 'escalation_action'
    ]
...
def render_system_health():
    ...
    for idx, (col, (name, status, is_online)) in enumerate(zip([col1, col2, col3, col4], health_status)):
        with col:
...
All violation data has been captured and exported.
            """
            st.success("✅ Report generated! Export the data using the buttons above.")
...
severity_filter, behavior_filter, date_range = render_filter_panel()
